...
var fillH = color.replace("*","H");
var fillV = color.replace("*","V");

trace("fillH:" + fillH + "/fillV:" + fillV);
...
createClipPattern(rectName);
	...
	//	punchX  = roundToSixteenth(punchX);
		...
		gv.getStyle().setProperty("fill",fillV,"");
//		gv.getStyle().setProperty("stroke-width",".0625","");
	...
	//	punchY = roundToSixteenth(punchY);
		...
		gh.getStyle().setProperty("fill",fillH,"");
...
var desc,grp,ins, t,punchX,punchY;
	...
	for(;nLeft <= left;nLeft++)
	{
		...
		punchX = Math.round(punchX*1000)/1000;
//		punchX  = roundToSixteenth(punchX);
		gl.getStyle().setProperty("fill",fillV,"");
...
//		punchX  = roundToSixteenth(punchX);
		...
		gr.getStyle().setProperty("fill",fillV,"");
	...
	{	
		...
		gt.getStyle().setProperty("fill",fillH,"");
		gt.setAttribute("x",x); 
	...
	{	
		...
		gb.getStyle().setProperty("fill",fillH,"");
		gb.setAttribute("x",x); 
...
trace("initOuterMiterFrame('"+id+"',"+x+","+ y+","+ w+","+ h+","+ fLeft+","+ fRight+","+ fTop+","+ fBot+",'"+ color+"')");
	...
	if(fBot <= fLeft)
	{
		path += "L "+(x + fBot)+","+(y+h-fBot)+" l -"+fBot+","+fBot;
	}
	else
	if(fBot > fLeft)
	{
...
//                var fillPattern = (color.charAt(0)==="*");
//                
//                if (fillPattern){
//                    color = replace
//                }
 ...
 */				
    try
    {
        ...
        if(mr === null)
        {
            ...
            mr.getStyle().setProperty("fill",(mr_w > mr_h ? fillH : fillV),"");
            grp.appendChild(mr);
        }
        else
        {
...
try
{
    ...
    with(grid){
        color = getGridColor(color,g("COLTYPE"));
        switch(pattern){
            case "9LP":
                //create grid
